...
/// <summary>
/// External adapter for CutRite — a CLI-based cutting optimizer (Wine compatible).
/// Submits CSV via <see cref="CliWrapperTransport"/> and polls for CSV result.
/// GetExecutionStatus and GetWasteReport are not supported by CutRite.
/// </summary>
internal sealed class CutRiteAdapter : ICuttingProvider
{
    private readonly IExternalAdapterTransport _transport;
    private readonly CutRiteFormatConverter _converter;
    private readonly IAdapterCallAuditWriter _auditWriter;
    private readonly ICuttingTenantAccessor _tenantAccessor;
    private readonly ILogger<CutRiteAdapter> _logger;

    public CutRiteAdapter(
        IExternalAdapterTransport transport,
        CutRiteFormatConverter converter,
        IAdapterCallAuditWriter auditWriter,
        ICuttingTenantAccessor tenantAccessor,
        ILogger<CutRiteAdapter> logger)
    {
    ...
    public async Task<Guid> SubmitCuttingSheetAsync(CuttingSheetDto sheet, CancellationToken ct = default)
    {
        ...
        await _auditWriter.RecordSubmitStartedAsync(callId, "cutrite", nameof(SubmitCuttingSheetAsync), tenantId, ct)
        ...
        try
        {
            ...
            if (!submitResult.IsSuccess)
            {
                ...
                throw new InvalidOperationException(
                    $"CutRite transport submit failed: {string.Join("; ", submitResult.Errors)}");
            }
            ...
            _logger.LogInformation(
                "CutRite sheet {SheetId} submitted for tenant {TenantId}, correlationId={CorrelationId}",
    ...
    public async Task<PanelAssignmentDto> GetNestingResultAsync(Guid sheetId, CancellationToken ct = default)
    {
        ...
        {
            _logger.LogDebug("CutRite nesting result not ready for sheet {SheetId}", sheetId);
        ...
        {
            _logger.LogError("CutRite result parse failed for sheet {SheetId}: {Errors}",
